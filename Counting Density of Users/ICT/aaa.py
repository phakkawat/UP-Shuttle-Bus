...
microgear.setalias("StatusMonitorICT")
    ...
    while xLoop < 400:
        ...
        if (Dens >= 2):
            if (Dens <= 50):
                ...
                print Image1, Image2, ',', ',', '%d' % NumberN
        if (NumberN < 1):
            microgear.chat("HTMLMonitorICT", "/0")
            print 0
        if (NumberN >= 1):
            if (NumberN < 4):
                microgear.chat("HTMLMonitorICT", "/1")
                print 1
        if (NumberN >= 4):
            if (NumberN < 8):
                microgear.chat("HTMLMonitorICT", "/2")
                print 2
        if (NumberN >= 8):
            microgear.chat("HTMLMonitorICT", "/3")
            print 3
        xLoop += 1
        zCount += 1
